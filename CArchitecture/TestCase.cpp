...
void CTestCaseCpp::mergeParts()
{
	...
	while(1)
	{
		...
		for(int i=0;i<partnum;i++)
		{
			...
			{
				if(EdgesBetweenParts[i][j]>maxCutEdge)
				{
					m = i;
					n = j;
					maxCutEdge = EdgesBetweenParts[i][j];
				}
			}
		}
		if(maxCutEdge>1)
		...
		for(int i=0;i<partnum;i++)
		{
			for(int j=i+1;j<partnum;j++)
			{
				if(EdgesBetweenParts[i][j]>SecmaxCutEdge&&i!=m&&j!=n)
				{
					mm = i;
					nn = j;
					SecmaxCutEdge = EdgesBetweenParts[i][j];
				}
			}
		}
		if(SecmaxCutEdge>1)
			mapForLastPart.insert(pair<int,int>(nn,mm));
	}
