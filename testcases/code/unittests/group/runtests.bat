...
@echo OFF
perl -e "$ENV{'EPOCROOT'} =~ m/([^\\]*)\\$/; if ($1 =~ m/3rd/i) {exit 0;} else {exit 1;}"

if %ERRORLEVEL%==0 GOTO WINSCW
if %ERRORLEVEL%==1 GOTO WINS
GOTO END

:WINSCW
echo WINSCW
start /min %EPOCROOT%\epoc32\release\winscw\udeb\symbianosunit.exe
goto END

:WINS
echo WINS
start /min %EPOCROOT%\epoc32\release\wins\udeb\symbianosunit.exe
goto END
