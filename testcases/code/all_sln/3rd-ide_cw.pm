...
my $thumbOptions = '--thumb  --fpu '.$floatingpointmodel.' --exceptions --exceptions_unwind';
my $armOptions = '--arm  --fpu '.$floatingpointmodel.' --exceptions --exceptions_unwind';
my $kernelOptions = '--arm --no_exceptions --no_exceptions_unwind';
my $invariantOptions = '--cpu 5T --enum_is_int -Ono_known_library --fpmode ieee_no_fenv --export_all_vtbl --no_vfe --apcs /inter';
...
sub Set_Compiler_Options() {
        my %plat = (main::PlatRec());
	foreach my $option (@{$plat{'CUSTOMIZATION_DATA'}}) {
...
sub ComputeCompilerOpts() {
	my %plat = &main::PlatRec();
	Set_Compiler_Options() if ($plat{'CUSTOMIZES'});
	my $ABI=&main::ABI;
	if (SysTrg()) {
	        $contingentOptions = $kernelOptions;
        } elsif (main::BuildAsARM() or ($ABI eq 'ARMV4')) {
	        $contingentOptions = $armOptions;
        } else {
	        $contingentOptions = $thumbOptions.' -D__MARM_THUMB__';	
	}
	...
	return $commonOptions.' '.$contingentOptions.' '.&main::CompilerOption("ARMCC").' '.$invopts;
}
...
sub addUserSearchPaths($) {
	...
	if ($DefFile && !(&main::ExportUnfrozen))
		{			
...
sub addSystemSearchPaths($) {
	...
	if (((&main::Plat =~ /^ARMV5$/) || (&main::Plat =~ /^ARMV5_ABIV2$/) || (&main::Plat =~ /^GCCE$/) ||(IsCustomization(&main::Plat))) && $PrefixFile) 
		{
	...
	if (&main::Plat =~ /^WINSCW$/)
	{
		...
		&textSetting($pathdata, "Path", $CW_libpath);
...
sub PMBld() {
	...
	if (($Plat eq "ARMV5") && ($RVCTMajorVersion == 2 && $RVCTMinorVersion < 2)) {
	...
	if ($Plat eq "GCCE" || $Plat eq "ARMV5_ABIV2") {
	...
	if (($Plat eq "ARMV5_ABIV2") && ($RVCTMajorVersion == 2 && $RVCTMinorVersion < 2))
	{
		die "FATAL ERROR: Target ARMV5_ABIV2 requires RVCT version 2.2 and greater. Detected RVCT $RVCTMajorVersion.$RVCTMinorVersion.\n";
	}
	
	if (($RVCTMajorVersion == 2 && $RVCTMinorVersion >= 2) && $CW_major_version < 3.1 && ($Plat eq "ARMV5_ABIV2"))
	{
...
#		$HelperLib='up_thumb.lib';
	...
	elsif ($Plat ne 'WINSCW' && $Plat ne 'ARMV5' && !$IsPlatCustomization && $Plat ne 'GCCE' && $Plat ne 'ARMV5_ABIV2') { 
...
#  		addFile($rssfile, "Text", "", "", "Resources");
	...
	foreach my $ResourceRef (@$ResourceStructRef) {
		...
		if (!defined $resourcefiles{$fullsource})
			{
			...
			$resourcefiles{$fullsource} = $entry;
			$resourcetargets{$fullsource} = $$ResourceRef{TrgPath}.&main::Path_Split('Base', $rssfile);

#			addFile($rssfile, "Text", "", "", "Resources");
			}
		$resourcefiles{$fullsource} .= "    <language id = \"$$ResourceRef{Lang}\"/>\n";
		push @ResourceDownloadList, $resourcetargets{$fullsource}.".R".$$ResourceRef{Lang};
	}

	foreach my $ResourceRef (@$ResourceStructRef) {
			my $fullsourcekey=$$ResourceRef{Source};
			$ResourcesText .= $resourcefiles{$fullsourcekey} . 
			" </rsc>\n";
			}
	...
	if ($Plat eq "WINSCW")
		...
		if ($TrgType eq "EXE")
			...
			# For S60 3.0, all EXEs will default to running epoc.exe. Users will need to launch the EXE themselves.
	...
	push @MacroList, "__SUPPORT_CPP_EXCEPTIONS__" if (($Plat eq "ARMV5") || ($Plat eq "ARMV5_ABIV2") || ($Plat eq "GCCE") || $IsPlatCustomization);
	...
	if ((($Plat eq "ARMV5") || ($Plat eq "ARMV5_ABIV2") || ($Plat eq "GCCE") || $IsPlatCustomization) && $VariantFile)
		{
...
#############################
	...
	elsif ((($Plat eq "ARMV5") || ($Plat eq "ARMV5_ABIV2") || $IsPlatCustomization) && ($CW_major_version >= 3)) 
		{
		...
		$compilerargs .= ComputeCompilerOpts();
...
############################
		...
		$compilerargs .= " -pipe -nostdinc -Wall -Wno-ctor-dtor-privacy -Wno-unknown-pragmas ";

		$compilerargs .= &main::CompilerOption("GCC");
...
############################
	...
	elsif ((($Plat eq "GCCE"))) # || $IsPlatCustomization) && ($CW_major_version >= 3)) 
		{
		$compiler = "ARM GCCE";
		...
		if ($Bld =~ /REL$/)
			{
		# 	gcc version 3.4.3 (release) (CodeSourcery ARM Q3cvs 2004) -- currently link fails when building with -O4
		#	$compilerargs .= " -O4 "
			}
		elsif ($Bld =~ /DEB$/)
			{
		...
		$compilerargs .= $GCCE_CompilerOption; #ComputeCompilerOpts();
		print $compilerargs,"\n";
		$changedsettings{"PrefixFile"} = &main::Path_Split('File',$PrefixFile);
		}
####################
...
############################################
	...
	if ($Plat eq "WINSCW")
		...
		if ($BasicTrgType =~ /^DLL$/o || $TrgType =~ /^EXEXP$/o)
			...
			{
			unless ($BaseTrg~=/tracealloc/) {
				$commonLinkFlags .= ' -m "?_E32Bootstrap@@YGXXZ"';
			} else {
				$commonLinkFlags .= ' -m "?_TraceStartup@@YGXXZ"';
			}
			}
...
##########################################
	...
	elsif ((($Plat eq "ARMV5") || $IsPlatCustomization) && ($CW_major_version >= 3)) 
		{
		...
		$Link = 'armlink '.$oP.'diag_suppress 6331,6780';
		...
		if ($BasicTrgType!~/^LIB$/o) {
			# Fix the "via" file for use with build 416 of ARMLINK - a defect currently exists where two adjacent words in a
			# build 416 ARMLINK via file lead to a buffer overload.  fixviafile.pl pops a benign '-diag_error' argument between
			# words that exceed this limit.  This workaround can be removed when ARMLINK is fixed.
			$linkCommand = 'perl -S fixviafile.pl "${target.data}\\${output.file.root}_'.$Bld.'_objects.via"';
			addLinkDescriptorCommand ($linkCommand);
...
#		    If platform is a customization, take libs from parent directory.		   
		    if($IsPlatCustomization)
		    {
			$linkCommand .= $oP.'entry _E32Dll "${var:KIT_EPOCROOT}\\EPOC32\RELEASE\\'.$ParentPlat.'\\'.$Bld.'\\'.$FirstLib.'('.$ObjFile.')"'.
			...
			if($IsPlatCustomization) 
			{
				$linkCommand .= $oP.'entry _E32Startup "${var:KIT_EPOCROOT}\\EPOC32\RELEASE\\'.$ParentPlat.'\\'.$Bld.'\\'.$FirstLib.'('.$ObjFile.')"';
			} 
			else
			{
			    	$linkCommand .= $oP.'entry _E32Startup "${output}\\'.$FirstLib.'('.$ObjFile.')"';
			}
			...
			$linkCommand = 'armar '.$oP.'create "${output}\\${output.file.name}" '.$oP.'via "${target.data}\\${output.file.root}_'.$Bld.'_objects.via"';
			addLinkDescriptorCommand ($linkCommand);
			}
		}
...
##############################################
	...
	elsif ((($Plat eq "ARMV5_ABIV2") || $IsPlatCustomization) && ($CW_major_version >= 3.1)) {
		...
		if ($BasicTrgType=~/^LIB$/o) {
	        $linkCommand = 'armar --create ${output}\\${output.file.name} ${var:LINK_OBJS} ${var:LIBS}';
	        addLinkDescriptorCommand ($linkCommand);
         }
		else
		{
...
#		addLinkDescriptorSymbol ('${var:IMPLIBS_NO_PATH_VTBLEXPORT}', $implibs_no_path_vtblexport);
		...
		$Link = 'armlink '.$oP.'diag_suppress 6331';
...
#		    If platform is a customization, take libs from parent directory.		   
		    ...
		    $linkCommand .= $oP.'entry _E32Dll "${var:KIT_EPOCROOT}\\EPOC32\RELEASE\\ARMV5\\'.$Bld.'\\'.$FirstLib.'('.$ObjFile.')"';
			#$linkCommand .= $oP.'entry _E32Dll "${output}\\'.$FirstLib.'('.$ObjFile.')"'.
			#		' "${target.data}\\'.$tempFilenameRoot.'.exp"';
		    }
		   ...
		   $linkCommand .= $oP.'entry _E32Startup "${var:KIT_EPOCROOT}\\EPOC32\RELEASE\\ARMV5\\'.$Bld.'\\'.$FirstLib.'('.$ObjFile.')"';
...
#			$linkCommand .= ' "${target.data}\\${output.file.root}.in"';
			...
			# use ksrt for system code and for user ARM code
			$StaticRTLib = "ksrt".$RVCTVersion if ($SystemTrg);
			# use the 'a' variant to get non-bpabi interpretation of .init_array
			$StaticRTLib .= "a" if ($RVCTMajorVersion == 2 && $RVCTMinorVersion >= 2);
			$runtimeLibs .= $StaticRTLib.".lib" unless ($Trg =~ /(U|K)SRT/i || ($BasicTrgType=~/^LIB$/o));
...
###############################################
	...
	if ($addHeaders)
		...
		if (($Plat eq "ARMV5" || $Plat eq "ARMV5_ABIV2" || $Plat eq "GCCE" || $IsPlatCustomization) && $PrefixFile) 
			{
