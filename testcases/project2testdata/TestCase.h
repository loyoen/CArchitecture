...
{
public:
	map <string, DWORD>		m_TimeRecord;
	typedef map < string, LPMETHOD >::const_iterator m_CIT;	// 定义迭代器便于查找map内容
	typedef map < string, string >::const_iterator m_CIT_p; // 查找类名与其对应的类指针的map内容
	...
	map <string, string> m_pMapTmp;							// 用于将testfile中的map对象取出，有可能不止一个，这里先只考虑一个
	LPTESTFILE			m_pFile;
...
public:
	...
	LPSTEDGE			m_pstEdge;
	// 超图上的超边
	LPEDGE				m_pHeadOnGraph;
	LPEDGE				m_pEdgeOnGraph;
	// 形成超图部分
...
public:
	...
	void		outputTimeLog ( string fileName );
	//void		HypergraphVertex();
