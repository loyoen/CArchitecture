...
{
	fathergraph = NULL;
	pAllVertices = (Vnode*)malloc(sizeof(Vnode)*(p->nvtxs));
	pAllEdges  = (HEnode*)malloc(sizeof(HEnode)*(p->nhedges));
	nvetes = p->nvtxs;
	nhedges = p->nhedges;
	maxHweight = 1;
	...
	for(int i=1;i<=p->nhedges;i++)
	{
		...
		{
			ArcNodeV *newvert = (ArcNodeV*)malloc(sizeof(ArcNodeV));
			...
			ArcNodeH *newedge =(ArcNodeH*)malloc(sizeof(ArcNodeH));
...
CMygraph* CMultilevel::coarseningGraph(CMygraph *initGraph)
{
	...
	MyHashStruct *pMyHash = (MyHashStruct*)malloc(sizeof(MyHashStruct)*(initGraph->maxHweight+1));
	...
	{		
		MyHashStruct *newone = (MyHashStruct*)malloc(sizeof(MyHashStruct));
		newone->truePos = i;
	...
	myCoarseningGraph->pMyHashForVert = (MyHashStruct*)malloc(sizeof(MyHashStruct)*(initGraph->nvetes)+1);
	...
	myCoarseningGraph->pAllVertices = (Vnode*)malloc(sizeof(Vnode)*vertCount);
	myCoarseningGraph->nvetes = vertCount;
	myCoarseningGraph->pAllEdges = (HEnode*)malloc(sizeof(HEnode)*edges_queue.size());
	myCoarseningGraph->nhedges = edges_queue.size();
	...
	for(int i=0;i<edges_queue.size();i++)
	{
...
void CMultilevel::FindZeroNodeCount(vector<int> cutEdges,int *PartZero,int *part_res,Vnode *pCutNodes)  //partzero[i] 代表超边i在的part0的顶点数
{
	for(int i=0;i<cutEdges.size();i++)
	{
		...
		while(myvert!=NULL)
		{
			...
			ArcNodeH *p = (ArcNodeH*)malloc(sizeof(ArcNodeH));
			p->hedge = edge;
...
double CMultilevel::myFM(int *part_res)
{
	...
	Vnode *pCutNodes = (Vnode*)malloc(sizeof(Vnode)*(pInitGraph->nvetes));
...
void CMultilevel::NodeRank(std::vector<int> cutNodes, std::vector<int> cutEdges, int *Node_Rank)
{
	...
	mymap *NodeMap = (mymap*)malloc(sizeof(mymap)*(max+1));
	...
	for(int i=0;i<cutEdges.size();i++)
	{
		mymap *newone = (mymap*)malloc(sizeof(mymap));
...
double CMultilevel::HillClimbing(int *part_res,vector<int> cutNodes,vector<int> cutEdges,int *Node_Rank,int *PartZeroNodeCount,Vnode *pCutNodes) //Node_Rank 值越小，越优先
{
	...
	int iDistance = 20;
	stack<Stone*> Hill;
	Stone *firstStone = (Stone*)malloc(sizeof(Stone));
	...
	int HeapLen = cutNodes.size();
	ModStruct *pModularityList = (ModStruct*)malloc(sizeof(ModStruct)*cutNodes.size());
...
void CMultilevel::HillClimbingBigEdgeFirst(int *part_res,vector<int> cutNodes,vector<int> cutEdges)
{
	...
	int step = 0;
	map<int,bool> ExitInList;
	while(step<iDistance&&step<cutNodes.size())
	{
		for(int i=max;i>=0;i--)
		{
			if(NodeMap[i].next == NULL)
			...
			while(pickone!=NULL)
			{
				...
				while(mynode!=NULL)
				{
					...
					if(step>=iDistance||step>=cutNodes.size())
						break;
					mynode = mynode->nextarc;
				}
				if(step>=iDistance||step>=cutNodes.size())
		...
		if(cutedge==false)
		{
			...
			while(pNode!=NULL)
			{
				...
				if(myiter==mapForInnerNodeCount.end())
				{
					...
					innerVertCount[part_num] += pInitGraph->pAllVertices[pNode->node].Vweight;
				}
				pNode = pNode->nextarc;
			}
